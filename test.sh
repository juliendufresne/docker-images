...
if [[ -v TRAVIS ]]; then
    (set +x; docker build --cache-from "$DOCKER_ORGANIZATION/$DOCKER_IMAGE" --tag "$DOCKER_ORGANIZATION/$DOCKER_IMAGE:$DOCKER_IMAGE_TAG" $DOCKER_IMAGE/$DOCKER_IMAGE_TAG)
else
    (set +x; docker build --tag "$DOCKER_ORGANIZATION/$DOCKER_IMAGE:$DOCKER_IMAGE_TAG" $DOCKER_IMAGE/$DOCKER_IMAGE_TAG)
fi
